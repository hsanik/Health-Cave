...
export async function POST(request) {
  ...
  // For development, skip signature verification if webhook secret is not set
  if (!endpointSecret || endpointSecret === "whsec_your_webhook_secret") {
    console.log(
      "Webhook secret not configured, parsing event without verification"
    );
    try {
      ...
      try {
        ...
        if (appointmentId) {
          ...
          const response = await fetch(
            `${backendUrl}/appointments/${appointmentId}/payment`,
            {
              method: "PUT",
              headers: {
