  ...
  try {
    ...
    try {
      ...
      if (doctorResponse.ok) {
        ...
        if (!data.isDoctor) {
          return NextResponse.json({ message: 'Access denied. Only doctors can manage availability.' }, { status: 403 })
        }
    ...
    const backendResponse = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/doctors/${doctorId}/availability`, {
      method: 'PUT',
      headers: {
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json({ message: error.error || 'Failed to update availability' }, { status: backendResponse.status })
    }
