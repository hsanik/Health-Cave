...
export async function POST(request) {
  try {
    ...
    const addVariant = (s) => { if (s && s.trim()) variants.add(s.toLowerCase().trim()) }
    ...
    if (process.env.NEXT_PUBLIC_BACKEND_URL) {
      try {
        const resp = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/doctors`)
        if (resp.ok) {
      ...
      } catch (e) { console.error('search-doctors backend error', e) }
    }
    ...
    const results = doctors
      .filter(d =>
        ...
        (() => { const spec = (d.specialty || '').toLowerCase(); for (const v of variants) { if (v && spec.includes(v)) return true } return false })()
      )
      .slice(0, safeLimit)

    const normalized = results.map(d => ({ id: d.id, name: d.name, specialization: d.specialty }))
    return NextResponse.json({ doctors: normalized })
  } catch (e) {
